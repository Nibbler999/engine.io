...
function Server (opts) {
  ...
  this.enforcePingTimeout = false !== opts.enforcePingTimeout;
  ...
  this.binaryType = opts.binaryType || 'nodebuffer';
...
Server.prototype.init = function () {
  ...
  if (~this.transports.indexOf('tcp')) {
    var TcpServer = require('http-tcp-packets').Server;
    this.tcp = new TcpServer({
      limit: this.maxHttpBufferSize,
...
Server.prototype.handleUpgrade = function (req, socket, upgradeHead) {
  ...
  this.verify(req, true, function (err, success) {
    ...
    if (req.headers.upgrade === 'http-tcp-packets') {
      self.tcp.handleUpgrade(socket, function (conn) {
        self.onUpgrade(req, conn);
      });
    } else {
      self.ws.handleUpgrade(req, socket, upgradeHead, function (conn) {
        conn.binaryType = self.binaryType;
        self.onUpgrade(req, conn);
      });
    }
  });
...
Server.prototype.onUpgrade = function (req, socket) {
...
Server.prototype.attach = function (server, options) {
  ...
  if (~self.transports.indexOf('tcp') || ~self.transports.indexOf('websocket')) {
